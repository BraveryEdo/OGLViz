...
GLuint loadBMP_custom(const char* imagepath) {
	...
	if ((err = fopen_s(&file, imagepath, "rb")) != 0) {
		...
		char buf[95];//safe err length function is being a dick so got max err length + 1 as 95
...
// for compressed textures (custom make using 'the compressonator')
...
int main(int argc, char* argv[]) {
	...
	/*
	//same process as above but for colors
	GLuint colorbuffer;
	...
	*/
	...
	//same process as above but for uv
	GLuint uvbuffer;
